...
- [x] **Step 2: 运行检查并确认复现故障**

Run: `npm run build:electron && node scripts/verify-electron-bundle.mjs`

Expected: FAIL，错误包含 `contains bundled Electron marker`。

Result: FAIL，错误为 `dist-electron/main.js does not load Electron at runtime`，证明 Electron 运行时引用已被完整内联。

- [x] **Step 3: 外置 Electron 模块并添加检查命令**
...
- [x] **Step 1: 在 Windows CI 构建阶段执行边界检查**
...
Result: Run `29632309363` 构建成功；EXE 大小为 `100083125` bytes；SHA-256 为 `20b2c8d640eab1e21366fbcfff7a2d5df2cacc64a0e58e6667fc9f89005ed2f4`。
